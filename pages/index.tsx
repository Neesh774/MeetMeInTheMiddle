...
const Home: NextPage = ({
  ...
  useEffect(() => {
    ...
    if (
      parseInt(newQuery.time.slice(0, 2)) > 23 ||
      parseInt(newQuery.time.slice(2, 4)) > 59 ||
      parseInt(newQuery.time.slice(0, 2)) < 0 ||
      parseInt(newQuery.time.slice(2, 4)) < 0
    ) {
      return;
    }
    ...
    if (
      ...
      router
        ...
        .catch(() => console.error(router.query, newQuery));
    }
